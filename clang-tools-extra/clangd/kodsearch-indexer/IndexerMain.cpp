...
static llvm::cl::OptionCategory KodSearchClangCategory("kodsearch-indexer", R"(
  Creates a kodsearch index of an entire project based on a JSON compilation database. The resulting database is saved in the CWD as `db.sqlite`.
  $ kodsearch-indexer compile_commands.json
  )");
...
static llvm::cl::opt<IndexFileFormat>
    Format("format", llvm::cl::desc("Format of the index to be written"),
           ...
           llvm::cl::init(IndexFileFormat::SQLITE));
...
int main(int argc, const char **argv) {
  ...
  if (compilations.getAllFiles().size() == 0) {
    fprintf(stderr, "ERROR: failed to find compilations in compilation database\n");
    exit(1);
  }

  // Create all translation unit executor with max concurrency
  clang::tooling::AllTUsToolExecutor Executor(compilations, 0);
